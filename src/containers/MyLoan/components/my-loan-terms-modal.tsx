...
const LoanStandardTerms = () => {
  return (
    <VStack w={'100%'} spacing={'16px'} alignItems={'flex-start'}>
      ...
      <VStack w={'100%'} spacing={'10px'} alignItems={'flex-start'}>
        <Text textStyle={'pre-body-68'}>
          <strong>제4조(실명거래)</strong>
          <br /> ① 대부업자와 채무자 사이의 거래는 실명으로 한다. <br />②
          대부업자는 채무자가 본인임을 확인할 의무가 있다. <br />③ 대부업자와
          채무자 사이의 계약은 채무자 본인이 직접 체결하는 것을 원칙으로 하되,
          채무자의 대리인과 계약을 체결하는 경우에는 채무자가 발급받은
          인감증명서 또는 본인서명사실확인서(또는 전자본인서명사실확인서
          발급증)를 첨부한 위 임장에 의하여야 한다.
        </Text>
        <Text textStyle={'pre-body-68'}>
          <strong>제5조(약관의 명시.설명.교부)</strong> <br /> ① 대부업자는 이
          약관을 영업장에 비치하고, 채무자는 영업시간 중 언제든지 이를
          열람하거나 그 교부를 청구할 수 있다. <br />② 대부업자는 계약체결 전에
          이 약관 제7조를 포함한 중요내용을 채무자가 이해할 수 있도록 설명하고
          약관을 교부한다.
        </Text>
        <Text textStyle={'pre-body-68'}>
        ...
        <Text textStyle={'pre-body-68'}>
          ...
          <br />
          ① 대부업자는 관계법령이 정하는 이자율(연체이자율 포함)의 범위 내에서
          계약을 체결하기로 한다.
          <br /> ② 제1항의 규정에 의한 이자율을 산정함에 있어 사례금, 할인금,
          수수료, 공제금, 연체이자, 체당금, 그밖에 그 명칭에 불구하고 대부와
          관련하여 대부업자가 받는 것은 이를 이자로 본다. 다만 당해 거래의
          계약체결과 변제에 관한 부대비용으로서 관련 법령이 정한 사항은 그러하지
          아니하다.
          <br />
          ③ 대부업자가 제1항의 규정에 의하지 않은 대부계약을 체결한 경우 제1항에
          따른 이자율을 초과하는 부분에 대한 이자계약은 무효로 한다.
          <br /> ④ 채무자가 대부업자에게 제1항에 따른 이자율을 초과하는 이자를
          지급한 경우 그 초과 지급된 이자 상당금액은 원본에 충당되고, 원본에
          충당되고 남은 금액이 있으면 그 반환을 청구할 수 있다.
          <br /> ⑤ 대부업자가 선이자를 사전에 공제하는 경우에는 그 공제액을
      ...
      <VStack w={'100%'} spacing={'10px'} alignItems={'flex-start'}>
        <Text textStyle={'pre-body-68'}>
          ...
          행사ᆞ보전(해지 포함)에 관한 비용
          <br /> 2. 채무이행 지체에 따른 독촉 및 통지비용
          <br /> 3. 채무 및 보증채무와 관련된 증명서의 발급 비용
        </Text>
        <Text textStyle={'pre-body-68'}>
          ② 대부업자나 대부업자가 지정하는 자가 제1항 각 호의 업무를 처리하고 그
          비용을 채무자에게 청구하는 경우에는 실비를 초과할 수 없으며 소요비용이
          최소화되도록 하여야 한다.
        </Text>
        <Text textStyle={'pre-body-68'}>
          ③ 제1항에 의한 비용을 대부업자가 대신 지급한 경우에는, 대부업자는 이를
          즉시 채무자에게 통지하여 채무자가 이를 곧 변제하도록 하고 만일
        ...
        <Text textStyle={'pre-body-68'}>
          ...
          아울러 이관 받은 본점, 지역본부 또는 다른 영업점 소재지 지방법원을
          관할법원으로 하기로 한다.
        </Text>
      </VStack>
    </VStack>
