...
function PrepaymentApplicationComplete() {
  const { prepaymentData } = useSessionStorage()
  const { data: loanData } = useLoanRetrieveQuery({
    variables: {
      id: Number(prepaymentData?.loanId),
    },
    options: {
      enabled: !!prepaymentData?.loanId,
    },
  })
        ...
        <Box w={'100%'} px={{ base: 0, sm: '74px', md: '160px' }}>
          ...
          >
            ...
            >
              ...
              <Text textStyle={'pre-body-3'} color={'grey.10'}>
                {prepaymentData?.totalAmount?.toLocaleString() || 0}원
              </Text>
            </HStack>
            <HStack
              borderColor={'border.basic.1'}
